...
from agoodfriendalwayspayshisdebts import model
from rules import WithEventBus
from agoodfriendalwayspayshisdebts.events import PurchaseAddedEvent


class ParticipantTestCase(unittest.TestCase):
    def test_generates_an_uuid_upon_creation_if_none_is_provided(self):
        participant = model.Participant('Bob & Lea', 2, 'bob@email.com')
...
class EventTestCase(unittest.TestCase):
    ...
    def test_has_participants(self):
        ...
        event = model.Event('', participants)
    ...
    def test_an_event_is_emitted_when_a_purchase_is_added(self):
        event = model.Event('cool event', [model.Participant('Kim', 1, uuid='1')], uuid='id123')

        event.add_purchase(model.Purchase('1', 1, ['1'], 'label'))
    ...
    def test_conversion_to_bson(self):
        ...
        kim = model.Participant('Kim', 1)
        event = model.Event('Cool event', [bob, kim])
        event.purchases.append(model.Purchase(bob.id, 10, [bob.id], 'Gas'))
        expected_event = {
            '_id': event.id,
            'name': 'Cool event',
            'participants': [
